...
if File.exists?('user-data') && ARGV[0].eql?('up')
  ...
  data['coreos']['units'].each_with_index do |(key, value), index|
    if key["name"] == "etcd3.service"
      data['coreos']['units'][index]['content'] = data['coreos']['units'][index]['content'].gsub(/https:\/\/discovery.etcd.io\/[0-9a-zA-Z]+/, token)
     break
    end
  end
  yaml = YAML.dump(data)
